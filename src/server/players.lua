...
local function steamProfileId(steamid)
    ...
    return tostring(tonumber(steamid, 16))
end
...
function Server.players.all(details)
    ...
    function obj.filterOperators(src)
        return obj.filter(function(player)
            if player.id == src then return false end
    ...
    function obj.filterNearbyOperators(src, coords, range)
        local retval = {}
        local nearby = obj.filterByDistance(src, coords, range)
        for i = 1, #nearby do
...
function Server.players.single(netid, nearby)
    local player = QBCore.Functions.GetPlayer(netid)
    local citizenid;
    if player ~= nil then citizenid = player.PlayerData.citizenid end
    ...
    obj.nearby = {}
    obj.nearby.players = Server.players.all(false).filterByDistance(obj.id, obj.coords, 100)
    obj.nearby.count = #obj.nearby.players
    obj.nearby.admins = Server.players.all(false).filterNearbyOperators(obj.id, obj.coords, 100)
